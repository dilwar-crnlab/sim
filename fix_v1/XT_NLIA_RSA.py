...
class UpdatedXT_NLI_A_RSA_Algorithm:
    ...
    def _attempt_single_chunk_allocation(self, connection_id: int, bandwidth_demand: float,
        ...
        for path_links in link_paths:
            ...
            # This will print the GSNR for every available channel before the
            # algorithm makes a selection.
            # ==================================================================
            # print(f"\n--- GSNR Analysis for Path {path_links} (Demand: {bandwidth_demand} Gbps) ---")
            # for resource in acv:
            #     print(f"  - Checking Core {resource.core_index}, Channel {resource.channel_index}: "
            #           f"GSNR = {resource.gsnr_result.gsnr_db} dB, "
            #           f"Max Bitrate = {resource.gsnr_result.max_bitrate_gbps} Gbps")
            # print("----------------------------------------------------------")
            # ==================================================================
    ...
    def _finalize_successful_allocation(self, connection, allocation_result: AllocationResult, path_links: List[int]):
        ...
        if allocation_result.allocated_resources:
            ...
            self.print_network_allocation_status(
    ...
    def print_network_allocation_status(self, connection_id, allocated_core: int = None, allocated_channel: int = None):
        """Print comprehensive network allocation status"""
        
        print(f"\n📊 NETWORK STATUS after allocating Connection {connection_id}:")
        
        # Count allocations per link
        link_summary = {}
        for link_id in range(self.spectrum_allocation.num_links):
            occupied = np.count_nonzero(self.spectrum_allocation.allocation[link_id, :, :])
            total = self.num_cores * self.num_channels
            link_summary[link_id] = (occupied, total)
            util_pct = (occupied / total) * 100
            print(f"   Link {link_id}: {occupied}/{total} resources ({util_pct:.1f}%)")
        
        # Show core utilization across all links
        print(f"\n   Core Utilization Summary:")
        for core_idx in range(self.num_cores):
            core_occupied = 0
            core_total = 0
            for link_id in range(self.spectrum_allocation.num_links):
                core_occupied += np.count_nonzero(self.spectrum_allocation.allocation[link_id, core_idx, :])
                core_total += self.num_channels
            
            util_pct = (core_occupied / core_total) * 100 if core_total > 0 else 0
            marker = " ⭐" if core_idx == allocated_core else ""
            print(f"     Core {core_idx}: {core_occupied}/{core_total} channels ({util_pct:.1f}%){marker}")
        
        # Show band utilization
        print(f"\n   Band Utilization:")
        for band_name, channel_indices in self.band_channels.items():
            band_occupied = 0
            band_total = 0
            for link_id in range(self.spectrum_allocation.num_links):
                for core_idx in range(self.num_cores):
            ...
            util_pct = (band_occupied / band_total) * 100 if band_total > 0 else 0
            marker = " ⭐" if allocated_channel in channel_indices else ""
            print(f"     {band_name}-band: {band_occupied}/{band_total} ({util_pct:.1f}%){marker}")
        
        # Overall network utilization
        total_occupied = np.count_nonzero(self.spectrum_allocation.allocation)
        total_resources = self.spectrum_allocation.allocation.size
        overall_util = (total_occupied / total_resources) * 100
        print(f"\n   Overall Network: {total_occupied}/{total_resources} resources ({overall_util:.1f}%)")
